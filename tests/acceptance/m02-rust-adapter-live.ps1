...
if ($deadlineLatenessEntries.Count -ne 32) { throw "adapter smoke reported an incomplete deadline-lateness histogram: $line" }
$deadlineLatenessSamples = [long]0
for ($bucket = 0; $bucket -lt 32; $bucket++) {
    $parts = $deadlineLatenessEntries[$bucket].Split(':')
    if ($parts.Count -ne 2 -or $parts[0] -ne "$bucket" -or $parts[1] -notmatch '^\d+$') { throw "adapter smoke reported malformed deadline-lateness histogram bucket: $line" }
