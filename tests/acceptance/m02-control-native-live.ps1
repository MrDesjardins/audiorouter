...
$savedEnvironment = @{}
foreach ($name in @('AUDIOROUTER_ALLOW_LIVE_AUDIO', 'AUDIOROUTER_CAPTURE_ENDPOINT_ID', 'AUDIOROUTER_RENDER_ENDPOINT_ID')) {
    $savedEnvironment[$name] = [Environment]::GetEnvironmentVariable($name, 'Process')
}

try {
    if ([string]::IsNullOrWhiteSpace($CaptureEndpointId) -or [string]::IsNullOrWhiteSpace($RenderEndpointId)) {
    ...
    Write-Output 'Scope: exact existing endpoints, 500 ms graph delivery, and same-process start/stop; no persistent audio configuration.'
}
finally {
