...
try {
    ...
    $graphQuantumFrames = [int]([regex]::Match($line, 'graph_quantum_frames=(\d+)').Groups[1].Value)
    $graphDeadlineNs = [long]([regex]::Match($line, 'graph_deadline_ns=(\d+)').Groups[1].Value)
    if ($captureRate -le 0 -or $renderRate -le 0 -or $graphQuantumFrames -ne 128 -or $graphDeadlineNs -ne [long][math]::Ceiling(($graphQuantumFrames * 1000000000.0) / $captureRate)) { throw "adapter route reported an invalid negotiated-rate deadline: $line" }
    ...
    Write-Output ("M02 Rust adapter route passed: render='{0}' capture='{1}' capture_rate_hz={2} render_rate_hz={3} graph_quantum_frames={4} graph_deadline_ns={5} capture_frames={6} graph_blocks={7} scheduler_frames={8} routed_frames={9} processing_time_ns_total={10} processing_time_ns_max={11} processing_time_p999_upper_bound_ns={12} processing_time_histogram_samples={13} processing_time_histogram={14} deadline_misses={15} deadline_lateness_ns_total={16} deadline_lateness_ns_max={17} deadline_lateness_p999_upper_bound_ns={18} deadline_lateness_histogram={19}" -f $renderLabel, $captureLabel, $captureRate, $renderRate, $graphQuantumFrames, $graphDeadlineNs, $captureFrames, $graphBlocks, $schedulerFrames, $routedFrames, $processingTimeTotal, $processingTimeMax, $processingTimeP999, $processingTimeSamples, $processingTimeHistogram, $deadlineMisses, $deadlineLatenessTotal, $deadlineLatenessMax, $deadlineLatenessP999, $deadlineLatenessHistogram)
