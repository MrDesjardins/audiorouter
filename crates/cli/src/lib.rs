...
fn mcp_tools() -> Value {
    let mut tools = json!([
        ...
        { "name": "create_recorder", "description": "Create an unarmed file recorder under the approved recording root; requires recording scope and an idempotency key.", "inputSchema": { "type": "object", "properties": { "sessionId": { "type": "string", "minLength": 1 }, "nodeId": { "type": ["string", "null"], "minLength": 1 }, "recorderId": { "type": "string", "minLength": 1 }, "format": { "enum": ["wavPcm16", "wavPcm24", "wavFloat32", "flac16", "flac24"] }, "sequence": { "type": "integer", "minimum": 0 }, "channels": { "type": "integer", "enum": [1, 2] }, "sampleRate": { "type": "integer", "enum": [44100, 48000] }, "dither": { "type": "boolean" }, "queueCapacity": { "type": "integer", "minimum": 1, "maximum": 2048 }, "maximumChunksPerPass": { "type": "integer", "minimum": 1 }, "idempotencyKey": { "type": "string", "minLength": 1 } }, "required": ["sessionId", "recorderId", "format", "sequence", "channels", "sampleRate", "queueCapacity", "maximumChunksPerPass", "idempotencyKey"], "additionalProperties": false } },
