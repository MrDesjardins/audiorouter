...
| `devices.list`, `nativeEndpoints.prepare`, `nativeEndpoints.detach`, `nativeApplications.prepare`, `nativeEndpoints.pump`, `nativeDuplex.detach`, `nativeDuplex.pump`, `applications.list` | Paged endpoint/application descriptors and binding candidates; `nativeEndpoints.prepare` validates exact active capture/render IDs and prepares stopped clients under `deviceAdministration`. `nativeEndpoints.detach` and `nativeDuplex.detach` remove only the exact session's stopped native worker or project-driver bridge, allowing deliberate replacement. `nativeApplications.prepare` validates the selected PID, executable/path/creation identity, matching committed `applicationCapture` node, and exact stereo render endpoint before preparing stopped process-loopback capture. `nativeEndpoints.pump` drains a bounded packet budget for the exact running generation; `nativeDuplex.pump` independently bounds input and output work for an attached running duplex generation. Application entries include PID, executable basename, nullable verified full executable path when permitted, creation identity, observed audio activity/session counts, separate render/capture session counts, display names, and capture-session observations; `notObserved` is not a claim that protected or future capture is impossible. |
