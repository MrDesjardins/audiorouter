...
- **API-08 — Events.** Support `events.subscribe` with session/resource filters, event categories, and after-sequence cursor. State events have increasing sequence, backend epoch, resource revision, and operation ID. Retain 10,000 state events or 15 minutes, whichever bound is reached first. `nextSequence` is the last event inspected by the bounded page (or the latest retained sequence when the page is short), so a full page can be resumed without skipping retained events. A lost cursor returns `resyncRequired`; clients fetch a snapshot. Meter events are lossy/coalesced and not replayed.
...
| `devices.list`, `nativeEndpoints.prepare`, `nativeEndpoints.detach`, `nativeApplications.prepare`, `nativeEndpoints.pump`, `nativeDuplex.detach`, `nativeDuplex.pump`, `nativeRenderSources.pump`, `nativeMultiInputs.prepare`, `nativeMultiInputs.pump`, `applications.list` | Paged endpoint/application descriptors and binding candidates; `nativeEndpoints.prepare` validates exact active capture/render IDs and prepares stopped clients under `deviceAdministration`. `nativeEndpoints.detach` and `nativeDuplex.detach` remove only the exact session's stopped native worker or project-driver bridge, allowing deliberate replacement. `nativeApplications.prepare` validates the selected PID, executable/path/creation identity, matching committed `applicationCapture` node, and exact stereo render endpoint before preparing stopped process-loopback capture. `nativeMultiInputs.prepare` validates the exact active capture IDs against the committed physical-input source order, resolves any required pre-bound plugin stages, and prepares a stopped generation-bound multi-capture worker. `nativeEndpoints.pump` drains a bounded packet budget for the exact running generation; `nativeDuplex.pump` independently bounds input and output work for an attached running duplex generation; `nativeRenderSources.pump` drains bounded work from an attached standalone virtual render-source worker for its exact running generation; `nativeMultiInputs.pump` drains several exact capture workers into a generation-bound feeder and reports physical fan-out delivery when an explicitly prepared output owner is attached. Application entries include PID, executable basename, nullable verified full executable path when permitted, creation identity, observed audio activity/session counts, separate render/capture session counts, display names, and capture-session observations; `notObserved` is not a claim that protected or future capture is impossible. |
...
| `nativeMultiInputs.bindBranches` | Bind the exact ordered output-node IDs of a prepared multi-input graph to branch-local virtual, recording, and tool observers before worker start. |
| `sessions.start/stop` | Idempotent lifecycle operations with per-node/recorder outcomes. Start may include an optional validated `candidate` session for temporary native preview; it must match the saved base revision, remains in memory for that run, and does not change the persisted session. Candidate preview uses an already prepared single-endpoint worker; it does not prepare endpoints or broaden the caller's grant. Test Signal node Play/Stop controls use these same methods. |
...
| `virtualRoutes.list/replace` | Explicit cross-session bus routes; replacement is authorized, revision-checked, durable, and idempotent |
...
| `audioMedia.beginUpload/uploadChunk/finishUpload/delete`, `audioSources.transport` | Bounded WAV/MP3 import into opaque backend media storage; per-node play/pause/stop/status for a running Audio File source and play/stop/status for a running Test Signal source. Import and source playback never open arbitrary file paths from the audio callback. |
| `audioMedia.importTemporaryRecording` | Convert a completed WAV created by an explicitly started temporary-take recorder into 24-hour media; remove the exact temporary file and library row after validation. Requires `record` and enforces the existing 64 MiB / 120-second source limits. |
...
The privileged `nativeBridges.prepare` method publishes one exact project-driver
render-source/capture-sink lease pair for an enabled virtual bus and declared
graph generation. `nativeBridges.detach` and `nativeBridges.heartbeat` provide
explicit cleanup and control-thread lease refresh; neither operation installs or
loads a driver or activates a graph.
