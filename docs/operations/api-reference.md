...
| `nativePaths.prepare` | `deviceAdministration` | external operation; prepares every independent path of a saved session (one source or one Mixer, then a chain, then outputs; up to 8 sources and 8 outputs in total) from the exact `endpointId` stored on each Physical Input/Output node and the identity stored on each Application Capture node; a mono endpoint feeding a stereo node is duplicated to both channels; one render endpoint may serve several paths; start, pump, stop and detach use the `nativeMultiInputs` methods |
...
The mutating native pump polls for endpoint changes; if a changed or removed
endpoint is owned by its running worker, that worker is stopped and its staged
audio is reset before `devices.bindingInvalidated` is retained. Rebinding is
always explicit and must revalidate the exact endpoint identity and format.
