...
Status: SDK, DSP, and supervised single-/multi-bus native VST3 worker
boundaries are implemented and tested; bounded pitch streaming with
control-plane reset is exposed as a fixed-quantum graph node, while realtime
graph scheduling, sandbox acceptance, and physical-latency qualification remain
open. Prerequisite: M05. Outcome: extensible effects and pitch processing with
