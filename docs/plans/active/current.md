...
The latest pushed checkpoint is `0d3d3ada` (2026-09-09). The guarded M00-M08
...
- Next M06/PLUG-03 task: exercise repeated native owner failures through the
  bounded restart budget and quarantine transition, then measure callback
  timing and long-run behavior; retain the existing VST2 single-stream worker
  path and fail closed until physical-latency evidence exists.
