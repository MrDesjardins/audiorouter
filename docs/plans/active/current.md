...
- Hardened native process-loopback data validation: capture results now include accumulated 16-bit sample energy and require nonzero payload in read-data modes, preventing packet-only success from being mistaken for signal delivery. The updated probe builds successfully; runtime attribution remains intentionally unrun because it emits an audible test tone.
