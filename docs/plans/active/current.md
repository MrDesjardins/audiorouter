...
Continue [M00](../../milestones/M00-feasibility.md) by implementing the asynchronous process-loopback activation probe against the documented Windows API, then run only initialization/format checks without starting or reading audio. In parallel, obtain an authorized Visual Studio/WDK environment before attempting the driver prototype; no driver install, default change, or capture stream start is authorized by this plan. Keep the WSL inventory as supporting context only and do not claim the feasibility gate until native evidence is attached.
