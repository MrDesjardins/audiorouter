...
The active branch is currently pushed through `b4061a7e`; later commits after
...
- Next M00/M02 task: add bounded deadline-aware pumping and a native tap/recorder attachment to `WasapiSchedulerBridge`, then repeat live validation with rollback snapshots. Keep render carry exhaustion fail-closed and retain the production driver/callback/signing gates.
