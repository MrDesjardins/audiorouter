...
The latest pushed checkpoint is pending this restart-recovery checkpoint. The guarded M00-M08
...
- Next M06/PLUG-03 task: integrate deliberate restart and quarantine policy
  with the asynchronous native worker owner, including bounded late-result
  handling; retain the existing VST2 single-stream worker path and fail closed
  until callback timing and physical-latency evidence exists.
