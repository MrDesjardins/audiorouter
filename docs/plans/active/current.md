...
The active branch is currently pushed through `e9de7c0d`; later commits after
...
- Next M00/M02 task: add bounded recovery/stop-start soak coverage around the endpoint-owned bridge while retaining the per-quantum deadline schedule, render carry fail-closed behavior, and all production driver/callback/signing gates.
