...
The latest implementation checkpoint is `8933a1e6`; the native probe compile was
...
(M01 CLI), `1815f193` (Windows-audio regression), and `576b9008` (plan
synchronization). The latest working tree is clean and
...
- Connected the portable engine-to-recorder path on 2026-09-10 at pushed head
  ...
  interleaves finite processed samples without allocation, and safely returns
  rejected chunks; the WAV integration regression now exercises the engine tap
...
- Requalified the repository-owned VST2 state/legacy boundary on 2026-09-09:
  standard and legacy `main` fixtures passed processing and chunk-state
  restoration at 44.1, 48, and 96 kHz; nonfinite output was rejected; and
...
- Requalified the explicitly selected user-provided x64 TDR Nova and BUSTERse
  VST2 DLLs on 2026-09-09. Both passed contained processing at 44.1, 48, and
  96 kHz plus editor-thread containment, with unchanged fingerprints
  (`4dfc5973...4e33` and `58032534...61d3`). Environment variables were
...
- Closed a UI-05 inspector gap on 2026-09-09: bounded numeric processor
  ...
  accessibility coverage was added; the full UI suite passed 17 files/121
...
- Updated the API reference and stable API specification on 2026-09-10 for
...
- Added an opt-in frontend-owned Tauri RPC acceptance on 2026-09-09:
  ...
  after it has crossed the control pipe. The marker is now create-only and
  rejects symlinked parents. The shell unit suite passed 5/5 and strict
  all-target Clippy passed. The release shell rebuilt successfully. The disposable acceptance harness
...
- Extracted the M08 bounded x64 PE validator into
  ...
  valid-header case. PowerShell parsing, path-safety, and artifact-verifier
  regressions passed; a clean M08 run rebuilt the CLI, plugin worker, and
  native shell and passed preparation/manifest verification. Temporary output
  was removed; no installer, signing, driver, audio, or machine configuration
  action occurred.

- Requalified the standalone native-shell boundary on 2026-09-10: the locked
  shell test suite passed 2/2, including JSON-safe initialization-script
  encoding and authenticated named-pipe RPC forwarding, and strict all-target
...
- Extended the TDR Nova x64 VST3 multi-bus qualification on 2026-09-10 across
  the full supported rate matrix: the supervised multi-bus worker and
  asynchronous graph-staging tests each passed at 44.1, 48, and 96 kHz. The
  native worker was temporary and removed, the supplied module was used in
  place, and no plugin registration, audio stream, or machine configuration
...
- Rechecked the native-shell frontend invocation boundary on 2026-09-09 after
  ...
  `session.snapshot` through the official Tauri core surface, while the native
  shell retains the authenticated named-pipe forwarding command. Portable
  host tests and UI/build checks passed; headless WebView2 invocation remains
  a manual observation gate because no frontend RPC was observable in the
  embedded headless launch. No audio, driver, signing, or machine configuration
  action occurred.

- Synchronized M08 operational documentation on 2026-09-09 after the release
  executable architecture hardening: the checklist now records bounded x64 PE
  validation and current M04/M05/M07 test counts, and release notes identify
  the same packaging safeguard. Documentation validation passed for 51
  Markdown files and 163 local links, and the runbook-command regression passed.
  No runtime, installer, signing, driver, audio, or machine configuration
  action occurred.
...
- Requalified dedicated M04 DSP/recording acceptance on 2026-09-10 with
  ...
  processing, queue/recovery behavior, metadata, and file finalization. No
  audio device or machine configuration was accessed.
...
- Corrected the Tauri shell bridge configuration on 2026-09-09 by enabling the
  global Tauri core API surface required by `ui/src/main.tsx` to discover
...
Requalified `safe-all.ps1` again at `4df24674` on 2026-09-10 after adding the
M03 source-contract assertions. The project-owned driver build and callback
invariant checks, all portable M00–M08 acceptance steps, unsigned release
preparation, traceability, and documentation validation passed; cleanup again
removed 13 run-owned temporary children. No driver installation/loading,
signing-mode change, plugin/startup registration, audio stream, or persistent
machine audio configuration occurred.
...
Latest pushed checkpoint: `4037c4fb` (`docs: requalify M00 toolchain`). The
...
Latest pushed checkpoint: `4037c4fb`. Since the prior full acceptance, the
...
The repository VST2 state/legacy fixture matrix was requalified on 2026-09-10:
modern and legacy entry points, chunk-state restoration at 44.1/48/96 kHz,
non-finite-output rejection, crash containment, and hang containment all passed
in disposable workers. No plugin registration or audio configuration action
occurred.

The native VST3 worker matrix was requalified on 2026-09-10: isolated AGain
single-stream and auxiliary-bus processing, asynchronous graph staging,
bounded failure recovery, repeated-quantum timing, state restoration, finite
...
remain repository-local; no system installation, plugin registration, audio
stream, or machine configuration action occurred.
...
- Synchronized the standalone Tauri lockfile after the Windows-audio dependency addition at pushed head `aa68d70c`; the change was limited to adding the workspace engine dependency to the existing package graph. The subsequent clean `tests/acceptance/m08-release.ps1` run passed on 2026-09-10, including native shell release compilation, UI build, unsigned artifact preparation, SBOM checks, and artifact verification. Temporary release output was removed; no installer, driver, signing, plugin registration, audio stream, or persistent machine configuration action occurred.
- Added the explicitly selected `adapter-bridge` native probe mode and rollback-checked acceptance script at pushed head `2410c5b7`. Friendly-name discovery and exact-ID runs against the existing VB-Audio Cable endpoints passed on 2026-09-10: 50–51 packets, 23,936–24,480 captured frames, 187–191 processed quanta, zero dropped render frames, zero xruns, and unchanged media-device snapshots after stream teardown. The run used the built-in gain graph through `WasapiSchedulerBridge`; it changed no defaults, volume, mute, privacy, driver, signing, startup, plugin registration, or persistent audio configuration.
- The first post-hardening live rerun exposed intermittent render availability: 352 frames were not submitted even though capture and scheduler processing succeeded. Corrected this at pushed head `a1c60e88` by adding a bounded 64-quantum render carry queue that preserves partial submissions and fails closed on queue exhaustion. Windows-audio tests passed 38/38, strict Clippy and probe checks passed, and the repaired rollback-checked live run passed with 24,480 captured frames, 24,448 rendered frames, zero dropped frames, zero xruns, and an unchanged media-device snapshot.
- Added the combined deadline-aware scheduler/tap path at pushed head `5aae3634`: `RealtimeScheduler::process_once_with_tap_and_deadline` and `WasapiSchedulerBridge::pump_with_tap_and_deadline` now preserve one timeline boundary for processing, recorder-compatible tap delivery, and deadline accounting. Engine tests passed 93/93; Windows-audio and native probe checks passed with strict Clippy. The live VB-Audio run delivered 191 tap calls for 191 processed quanta, zero non-finite tap samples, zero deadline misses, zero xruns, zero dropped render frames, and unchanged media-device state.
- Requalified the complete guarded `tests/acceptance/safe-all.ps1` chain at pushed head `a666d1a3` on 2026-09-10: M00 toolchain/native compile and 31-endpoint inventory, disposable SysVAD x64 package/API/signability, M01, M04, M05 (17 UI files/121 tests), M06 SDK/native VST3 and VST2 fixtures, M07, unsigned M08 artifacts, 159 traceability mappings, and documentation validation (51 Markdown files/163 local links) all passed. Cleanup removed 13 run-owned temporary children. No driver installation/loading, signing-mode change, plugin/startup registration, audio stream, or persistent machine audio configuration occurred.
- Extended the guarded live bridge probe at pushed head `2702839c` with a real `StreamingFlacRecorderWorker` attached through the allocation-free tap. The VB-Audio run passed on 2026-09-10 with 51 packets, 24,480 captured frames, 191 processed/tapped quanta, 24,448 rendered frames, a finalized 25,072-byte FLAC artifact, zero non-finite tap samples, zero dropped render frames, zero xruns, zero deadline misses, and unchanged media-device state. The temporary recording and streams were removed after the run; no persistent audio configuration changed.
- Requalified the complete guarded `tests/acceptance/safe-all.ps1` chain at pushed head `b4061a7e` on 2026-09-10 after the live recorder attachment: M00 toolchain/native compile and 31-endpoint inventory, disposable SysVAD x64 package/API/signability, M01, M04, M05 (17 UI files/121 tests), M06 SDK/native VST3 and VST2 fixtures, M07, unsigned M08 artifacts, 159 traceability mappings, and documentation validation (51 Markdown files/163 local links) all passed. Cleanup removed 13 run-owned temporary children. No driver installation/loading, signing-mode change, plugin/startup registration, audio stream, or persistent machine audio configuration occurred.
- Hardened the live recorder probe cleanup at pushed head `bdb81488`: temporary FLAC recordings now use a collision-resistant name and an RAII cleanup guard, so early activation/finalization failures cannot leave probe artifacts. Probe compile, strict Clippy, diff checks, and the rollback-checked live bridge/recorder run passed again with 24,480 captured frames, 191 processed/tapped quanta, 25,072 finalized FLAC bytes, zero drops/xruns/deadline misses, and unchanged media-device state.
- Added rate-aware per-quantum deadline scheduling at pushed head `e9de7c0d` on 2026-09-10. `WasapiSchedulerBridge::pump_with_tap_and_quantum_deadline` advances each engine quantum deadline from the bridge timeline and sample-rate-derived quantum duration; the existing API remains available for callers that intentionally observe one deadline per bounded pump. The focused Windows-audio suite passed 39/39, strict Clippy, formatting, and native probe compilation passed. The authorized live VB-Audio Cable run passed with 76 packets, 36,480 captured/processed-tapped frames, 36,480 rendered frames, zero non-finite samples, drops, xruns, and deadline misses; media-device state was unchanged and temporary streams/recording were removed.
- Added bounded native stop/start soak coverage at pushed head `9362fe4b` on 2026-09-10: the guarded bridge acceptance now supports one to five independent cycles, checking telemetry and the complete media-device snapshot after every cycle. Three authorized 300 ms cycles passed against the existing VB-Audio Cable pair, each processing 116 tapped quanta and rendering 14,848 frames with zero drops, xruns, deadline misses, and unchanged device state. Temporary streams and recordings were removed after every cycle.
- Requalified the complete guarded M00-M08 chain at pushed head `d1c9f5af` on 2026-09-10: VS2026/WDK native compatibility and disposable SysVAD package/API/signability, M01 CLI, M04 DSP/recording, M05 UI (17 files/121 tests), M06 VST3 and VST2 fixtures/workers, M07 headless, unsigned M08 release preparation, 159 normative traceability mappings, and documentation validation (51 Markdown files/163 local links) all passed. The runner removed 13 run-owned temporary children. No driver installation/loading, signing-mode change, plugin/startup registration, audio stream, or persistent machine audio configuration occurred.
- Requalified the installed ReaPlugs VST2 directory on 2026-09-10. The first run intentionally exposed two non-audio candidates: `reacontrolmidi-standalone.dll` failed the audio-effect state contract and `reajs.dll` returned an incompatible state result. Re-running with the wrapper's explicit `-SkipIncompatibleCandidates` policy rejected those two candidates and passed all seven usable x64 audio DLLs across 44.1, 48, and 96 kHz (21 processing cases). The wrapper restored both VST2 environment variables; DLLs were not modified or registered, and no audio or machine configuration changed. This confirms the candidates are not device-contention failures and preserves the explicit audio-effect compatibility boundary.
- Attempted the M07 frontend-owned shell RPC acceptance on 2026-09-10. The guarded script stopped before WebView2/backend launch because the current PowerShell process is not elevated and this machine's shell setup requires administrator rights. No shell, audio endpoint, plugin, or persistent machine configuration was touched. The acceptance remains open until an administrator-authorized shell session is available; the portable UI/RPC contracts remain validated independently.
- Requalified native impulse correlation on 2026-09-10 using the explicitly selected VB-Audio Cable pair: 246 of 250 impulses were detected, p95 spacing error was 0 frames, and estimated digital-loopback onset was 67.40 ms. Streams and temporary artifacts were cleaned, and the media-device snapshot was unchanged. This is signal-path timing evidence only; calibrated physical acoustic latency remains open until a timestamped physical setup is available.
- Requalified native endpoint loopback on 2026-09-10 using the explicitly selected VB-Audio Cable pair: the 500 ms capture observed 71,212 nonzero payload bytes while an 800 ms temporary tone ran through the selected render endpoint. Both streams stopped/reset successfully, and defaults, volume, mute, privacy, driver, signing, startup, and media-device state remained unchanged. This proves a digital endpoint signal path, not calibrated physical acoustic latency or managed virtual-driver behavior.
- Requalified process-loopback exclusion on 2026-09-10 with a disposable child process: the explicitly guarded exclude-tree mode captured 32,634 frames over 750 ms, reported successful activation/start/stop/reset, and observed clean child exit. The media-device snapshot remained unchanged. This validates the Windows API mode and lifecycle only; full cross-process isolation thresholds and production virtual-device behavior remain separate gates.
- Requalified the guarded native endpoint lifecycle matrix on 2026-09-10: 13 capture endpoints and 18 render endpoints were exercised for 125 ms, with one occupied render endpoint correctly reported as an ownership condition. Capture and render clients completed their bounded lifecycle checks, and defaults, volume, mute, privacy, driver, signing, startup, and media-device state remained unchanged. This is shared-mode lifecycle evidence, not production driver or physical-latency evidence.
- Requalified process-loopback inclusion on 2026-09-10 with a disposable child process: the guarded include-tree mode captured 32,634 frames and 121,291 nonzero payload bytes over 750 ms, reported successful activation/start/stop/reset, and observed clean child exit. The media-device snapshot remained unchanged. This complements the exclusion-mode evidence while leaving production cross-process isolation and virtual-device gates open.
- Requalified event-driven WASAPI lifecycle on 2026-09-10 using the explicitly selected VB-Audio Cable pair: the event capture path received 36,480 frames and the silent event render path submitted 40,800 frames over 750 ms. Event registration, activation, start, stop, and reset all passed; defaults, volume, mute, privacy, driver, signing, startup, and media-device state remained unchanged. This is event-mode shared-client evidence, not managed-driver callback timing.
- Requalified the Rust process-loopback adapter on 2026-09-10 in both include and exclude modes. Each 500 ms run converted 22,050 source frames at 44.1 kHz into 23,936 engine frames at 48 kHz, processed 187 scheduler quanta, and reported zero rejected packets, xruns, or input/output underruns with 25 bounded resampler frames queued. Streams stopped/reset and the media-device snapshot remained unchanged. This is project-adapter evidence; protected-content behavior and managed virtual-device routing remain open.
- Requalified the broader Rust adapter live acceptance on 2026-09-10: the selected active endpoints delivered 36,960 capture frames into 288 generation-1 gain quanta and 37,536 rendered frames over 750 ms. Processing totaled 6,099,400 ns with a 56,500 ns maximum and 288 histogram samples; xruns, input/output overruns, deadline misses, and deadline lateness were all zero. The streams stopped/reset and media-device identity/state remained unchanged. This is shared-mode adapter telemetry, not production-driver callback timing.
- Requalified the locked workspace regression after the native adapter additions on 2026-09-10: workspace tests and doc-tests passed, strict all-feature Clippy passed with `-D warnings`, and documentation validation passed for 51 Markdown files and 163 local links. The Windows-audio suite passed 39 tests. This regression opened no endpoint and changed no driver, plugin registration, stream, or machine configuration.
- Qualified VST2 editor containment for the supplied Pitchproof binaries on 2026-09-10. The initial directory-wide run selected the x86 `pitchproof.dll` and correctly exposed `LoadLibrary(193)` (wrong architecture), not an editor fault. An explicitly isolated copy of `pitchproof-x64.dll` then passed both bounded non-returning-editor and supervised timeout/reaping tests. The temporary copy was removed, the environment was restored, and no plugin was registered or modified.
- Hardened `m06-vst2-editor.ps1` at pushed head `dd25cfb0` to inspect PE machine type before loading candidates, matching the processing wrapper's x64 boundary. The mixed Pitchproof directory now reports and skips x86 `pitchproof.dll` before loading; the x64 DLL passes both editor containment tests. This prevents architecture errors from being misclassified as editor or worker failures.
- Requalified the explicitly selected installed Pitchproof x64 VST2 binary on 2026-09-10: processing/state tests passed at 44.1, 48, and 96 kHz, both editor containment tests passed, and the SHA-256 remained `1974a3033b53ae72da5f419a9f37056d44c1610591bfd11a615ace0c448cf050`. Environment variables were restored; the DLL was not copied, modified, or registered, and no audio configuration changed.
- This is measured shared-mode WASAPI adapter evidence for the new graph path, not production virtual-device callback evidence. Remaining gates include managed virtual-device/driver ownership, callback deadline measurement under production ownership, recovery soak, physical latency, signing, installer, clean-machine qualification, and manual elevated-shell UI validation.
- Next M00/M02 task: add controlled endpoint-invalidation/rebind evidence using a testable device-lifecycle seam; do not disturb user defaults or production devices, and retain the per-quantum deadline schedule, render carry fail-closed behavior, and all production driver/callback/signing gates.
...
- Ran the read-only CLI scan of `C:\\Program Files\\VSTPlugins\\ReaPlugs` on 2026-09-09. It found nine x64 VST2 candidates, each reported as `supportedVst2X64Gated`; this is discovery identity only. The six audio effects (ReaComp, ReaDelay, ReaEQ, ReaFIR, ReaGate, and ReaXComp) retain runtime qualification, while MIDI/utility candidates are not treated as processing support until the worker validates their audio-effect header. No plugin was loaded or modified and no machine/audio configuration changed.
- Re-ran `tests/acceptance/m06-vst2-reaplugs.ps1` on 2026-09-09 against the six repository-local ReaPlugs effect DLLs. All 18 effect/rate combinations (44.1, 48, and 96 kHz) passed through disposable workers, including parameter-offset and before/after binary-integrity checks. The wrapper restored both `AUDIOROUTER_VST2_FIXTURE` and `AUDIOROUTER_VST2_SAMPLE_RATE`; no plugin registration, audio stream, or machine configuration changed.
...
- Revalidated the locked workspace after recorder lifecycle forwarding at
  ...
  changed.
- Revalidated the locked workspace after recorder timeline failure handling at
  pushed head `9a47f7cb` on 2026-09-11. Workspace tests passed, including
  control (111), recording (39), Windows-audio (62 with 11 expected native
  fixture skips), plugin-host (67), storage (80), and worker-process coverage.
  Workspace strict Clippy, formatting, and diff checks passed. No endpoint,
  driver, plugin registration, or persistent machine audio configuration
  changed.
...
- Revalidated the locked workspace after API stop finalization at pushed head
  ...
  plugin-host (67), storage (80), and worker-process coverage. Workspace
  strict Clippy, formatting, and diff checks passed. No endpoint, driver,
  plugin registration, or persistent machine audio configuration changed.
