...
The latest pushed checkpoint is `eb0ad978` (2026-09-09). The guarded M00-M08
...
- Next M06/PLUG-03 task: connect the supervised native VST3 worker to bounded
  graph scheduling and caller-owned realtime staging; retain the existing VST2
  single-stream worker path and fail closed until realtime and physical-latency
  evidence exists.
