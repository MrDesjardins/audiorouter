...
the pinned SDK-evidence head. It covered M00/M03 toolchain, native,
AudioRouter x64 and ARM64 source, Software Device dry-run, read-only 31-
endpoint inventory, disposable SysVAD, M01, M04, M05 (234 UI tests and
production builds), M06 VST3/VST2 SDK and worker gates, M07 (36 CLI, 3 MCP,
173 control, 70 plugin-host, and 13 worker-process tests), unsigned M08
artifact preparation, unsigned x64 NSIS smoke, 159 traceability mappings,
and 54 Markdown files with 218 local links. The generated installer was
`AudioRouter_0.1.0_x64-setup.exe` at 4,760,908 bytes. Cleanup removed 15
...
Signing, installation, clean-machine, physical-latency, actual transition
rebind, and attended UI/accessibility gates remain open.

## 2026-09-16 - elevated complete guarded acceptance at `d52f7ec4`

The elevated `tests/acceptance/safe-all.ps1` chain passed with exit code 0 at
the endpoint-invalidation UI head. It covered M00/M03 toolchain, native,
AudioRouter x64 and ARM64 source, Software Device dry-run, read-only 31-
endpoint inventory, disposable SysVAD, M01, M04, M05 (234 UI tests and
production builds), M06 VST3/VST2 SDK and worker gates, M07 (36 CLI, 3 MCP,
173 control, 70 plugin-host, and 13 worker-process tests), unsigned M08
artifact preparation, unsigned x64 NSIS smoke, 159 traceability mappings,
and 54 Markdown files with 218 local links. The generated installer was
`AudioRouter_0.1.0_x64-setup.exe` at 4,763,731 bytes. Cleanup removed 15
run-owned temporary children. No driver was installed or loaded, no plugin or
startup registration occurred, and no persistent audio configuration changed.
Signing, installation, clean-machine, physical-latency, actual transition
rebind, and attended UI/accessibility gates remain open.

## 2026-09-16 - elevated complete guarded acceptance at `4fb78ce3`

The elevated `tests/acceptance/safe-all.ps1` chain passed with exit code 0 at
the corrected read-only inventory/lifecycle head. It covered M00/M03
toolchain, native, AudioRouter x64 and ARM64 source, Software Device dry-run,
read-only 31-endpoint inventory, disposable SysVAD, M01, M04 (34 DSP and 40
recording tests), M05 (233 UI tests and production builds), M06 VST3/VST2 SDK
and worker gates, M07 (36 CLI, 3 MCP, 173 control, 70 plugin-host, and 13
worker-process tests), unsigned M08 artifact preparation, unsigned x64 NSIS
smoke, 159 traceability mappings, and 54 Markdown files with 218 local links.
The generated installer was `AudioRouter_0.1.0_x64-setup.exe` at 4,762,510
...
persistent audio configuration changed. Signing, installation, clean-machine,
physical-latency, actual transition rebind, and attended UI/accessibility
gates remain open.

## 2026-09-16 - elevated complete guarded acceptance at `16d3835e`

The elevated `tests/acceptance/safe-all.ps1` chain passed with exit code 0 at
the rebind/UI head. It covered M00/M03 toolchain, native, AudioRouter x64 and
ARM64 source, Software Device dry-run, read-only 31-endpoint inventory,
disposable SysVAD, M01, M04 (34 DSP and 40 recording tests), M05 (233 UI tests
and production builds), M06 VST3/VST2 SDK and worker gates, M07 (36 CLI, 3
MCP, 173 control, 70 plugin-host, and 13 worker-process tests), unsigned M08
artifact preparation, unsigned x64 NSIS smoke, 159 traceability mappings, and
54 Markdown files with 218 local links. The generated installer was
`AudioRouter_0.1.0_x64-setup.exe` at 4,763,039 bytes. Cleanup removed 15
run-owned temporary children. No driver was installed or loaded, no plugin or
startup registration occurred, and no persistent audio configuration changed.
Signing, installation, clean-machine, physical-latency, actual transition
rebind, and attended UI/accessibility gates remain open.
...
pushed head. It covered M00 toolchain/native checks and 34-endpoint inventory,
M03 AudioRouter WDK compile/signability/catalog plus disposable SysVAD,
M01/M04, M05 with 221 UI tests and production builds, M06 VST3/VST2 workers,
M07 headless controls, unsigned M08 artifacts, 159 normative mappings, and
documentation validation with 54 Markdown files and 210 local links. Cleanup
removed 15 run-owned temporary children. No driver was installed or loaded,
no signing mode or boot policy changed, and no plugin/startup registration or
persistent machine audio configuration changed. Live production-driver,
signing/install, physical-latency, and attended UI/accessibility gates remain
open.
...
VB-Cable readiness head. It covered the installed VS2026/WDK toolchain,
read-only 34-endpoint inventory, non-installing AudioRouter driver build and
catalog signability, disposable SysVAD, M01/M04, M05 (150 UI tests and
production build), M06 VST3/VST2 workers, M07, unsigned M08 artifacts, 159
requirement mappings, and documentation validation (52 Markdown files, 180
local links). Fifteen run-owned temporary children were removed.

This remains qualification evidence only: the driver was not installed or
loaded, no plugin or startup registration occurred, and no endpoint default,
volume, mute, or persistent machine-audio configuration changed. Attended
WebView2, production driver, signing, clean-machine, and physical-latency
gates remain open.
...
171 local links. Cleanup removed 15 run-owned temporary children. No driver
...
The complete elevated `tests/acceptance/safe-all.ps1` chain passed at pushed
head `f578b073` after the prototype endpoint-label update. M00 toolchain/native
checks, the non-installing AudioRouter driver build, read-only 31-endpoint
inventory, disposable SysVAD qualification, M01, M04 (30 DSP and 40 recording
tests), M05 (124 UI tests), M06 VST3 and VST2 workers, M07, unsigned M08
artifacts, 159 traceability IDs, and documentation validation (52 Markdown
files/167 local links) all passed. Cleanup removed 13 run-owned temporary
children. No driver installation/loading, signing or boot-policy change,
plugin/startup registration, live stream, or persistent machine audio
configuration change occurred. Production driver activation/signing,
installer, clean-machine, physical-latency, manual UI, and independent-plugin
...
## 2026-09-08 - Safe acceptance after editor authorization (`1e1ba0cb`)

The elevated `tests/acceptance/safe-all.ps1` chain passed at the current
working head after the VST2 editor parent-authorization change. It passed
toolchain discovery, native compile, read-only 34-endpoint format inventory,
disposable pinned SysVAD x64 compile/package/API qualification, M01 CLI, M04
DSP/recording, M05 UI (typecheck, 91 tests, and temporary production build),
M06 SDK/VST3, M07 headless, unsigned M08 artifacts, 159 normative
traceability mappings, and documentation validation (51 Markdown files, 161
local links). Temporary outputs/checkouts were removed. No driver was
installed or loaded, and no signing mode, plugin/startup registration, stream,
or machine audio configuration changed. This remains preparation evidence,
not signed release, production-driver, installer, clean-machine,
physical-latency, or manual-UI evidence.
