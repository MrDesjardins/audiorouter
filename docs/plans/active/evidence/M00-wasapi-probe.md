...
Read-data modes now also report accumulated 16-bit sample energy and require nonzero payload bytes for success. This closes the packet-count-only false-positive path while preserving silent-packet accounting. The change compiled successfully with temporary executable/object outputs removed afterward.
