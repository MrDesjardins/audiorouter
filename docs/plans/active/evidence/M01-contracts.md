...
- `crates/storage`: SQLite schema migration, session JSON documents, revision storage/history, and idempotent operation journal.
...
Still not evidenced: durable journal crash injection; persistent client enrollment/revocation; SQLite backup/import limits; named-pipe singleton/concurrency stress; real endpoint discovery/activation; process-tree capture; driver lifecycle; realtime callback safety; physical latency; and M02 hardware acceptance. M00 remains open and M01 is not a releasable product gate.
