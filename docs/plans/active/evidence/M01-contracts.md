...
The portable `EventLog` now establishes bounded state-event replay semantics: every event carries backend epoch, sequence, resource revision, optional operation ID, category, and optional session ID. Replay is limited to 500 records and returns `ResyncRequired` when retention has passed the requested cursor. It excludes meter data. The control dispatcher and authenticated transport consume this replay contract, including cursor, session, backend-epoch, and bounded category filters.

Control now owns an event log and exposes `events.subscribe` with optional `afterSequence`, `limit`, `sessionId`, and bounded `categories` filters. Session creation and graph commits append state events; control regressions verify epoch, ordering, operation ID, category/session filtering, invalid filter sizes, and cursor expiry. Authenticated persistent transport sessions preserve control-plane state across bounded request frames; an unbounded production daemon/subscriber lifecycle remains open.
