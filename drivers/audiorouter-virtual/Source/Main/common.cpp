    ...
    public CUnknown
{
    private:
        ...
        PCAudioRouterVirtualHW   m_pHW;                  // Virtual AudioRouter Virtual HW object
    ...
    public:
        ...
        (
            ...
            _In_reads_opt_(cPropertyCount) const AUDIOROUTERVIRTUAL_DEVPROPERTY   * pProperties,
...
NTSTATUS AudioRouterVirtualIoSetDeviceInterfacePropertyDataMultiple
(
    _In_ PUNICODE_STRING                                        SymbolicLinkName,
    _In_ ULONG                                                  cPropertyCount,
    _In_reads_opt_(cPropertyCount) const AUDIOROUTERVIRTUAL_DEVPROPERTY        *pProperties
...
--*/
{
    ...
    // Initialize HW.
    //
    m_pHW = new (POOL_FLAG_NON_PAGED, AUDIOROUTERVIRTUAL_POOLTAG)  CAudioRouterVirtualHW;
    if (!m_pHW)
    {
        DPF(D_TERSE, ("Insufficient memory for AudioRouter Virtual HW"));
        ntStatus = STATUS_INSUFFICIENT_RESOURCES;
    }
...
--*/
{
    ...
    //
    ntStatus = AudioRouterVirtualIoSetDeviceInterfacePropertyDataMultiple(AudioSymbolicLinkName, cPropertyCount, pProperties);

    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("CreateAudioInterfaceWithProperties: AudioRouterVirtualIoSetDeviceInterfacePropertyDataMultiple(...): failed, 0x%x", ntStatus)),
        Done);
...
)
{
    ...
    if (!NT_SUCCESS(ntStatus) || NULL == unknownTopology || NULL == unknownMiniTopo)
    {
        ...
        // Install AudioRouter Virtual topology miniport for the render endpoint.
        //
        ntStatus = InstallSubdevice(Irp,
                                    MiniportPair->TopoName, // make sure this name matches with AUDIOROUTERVIRTUAL.<TopoName>.szPname in the inf's [Strings] section
    ...
    if (!NT_SUCCESS(ntStatus) || NULL == unknownWave || NULL == unknownMiniWave)
    {
        ...
        // Install AudioRouter Virtual wave miniport for the render endpoint.
        //
        ntStatus = InstallSubdevice(Irp,
                                    MiniportPair->WaveName, // make sure this name matches with AUDIOROUTERVIRTUAL.<WaveName>.szPname in the inf's [Strings] section
