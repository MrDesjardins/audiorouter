param(
    [string]$Output = (Join-Path $PSScriptRoot 'm03-swdevice-probe.exe'),
    [string]$Object = ''
)

$ErrorActionPreference = 'Stop'
$vswhere = 'C:\Program Files (x86)\Microsoft Visual Studio\Installer\vswhere.exe'
if (-not (Test-Path -LiteralPath $vswhere)) { throw "Visual Studio discovery tool is missing: $vswhere" }
...
if ([string]::IsNullOrWhiteSpace($Object)) { $Object = [IO.Path]::ChangeExtension($output, '.obj') }
$object = [IO.Path]::GetFullPath($Object)
foreach ($path in @($cl, $vcInclude, $vcLib, "$include\um\Windows.h", "$include\um\swdevice.h", "$umLib\Swdevice.lib")) {
    if (-not (Test-Path -LiteralPath $path)) { throw "Required native toolchain path is missing: $path" }
}
& $cl /nologo /EHsc /std:c++20 "/I$vcInclude" "/I$include\shared" "/I$include\um" "/I$include\ucrt" $source /Fo:$object /Fe:$output /link "/LIBPATH:$vcLib" "/LIBPATH:$umLib" "/LIBPATH:$ucrtLib" Swdevice.lib
