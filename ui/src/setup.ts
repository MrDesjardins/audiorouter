...
export function setupChecklist(input: { connected: boolean; audio: string | null; storage: string | null; deviceCount: number; applicationCount: number; vbCablePairAvailable: boolean }): SetupStep[] {
  return [
    ...
    { id: "vb-cable", label: "VB-Cable pair", state: input.vbCablePairAvailable ? "ready" : input.connected ? "needs-attention" : "unavailable", detail: input.vbCablePairAvailable ? "Exact active CABLE Output/Input pair detected" : input.connected ? "Select the exact VB-Cable pair before preparing native audio" : "Unavailable until the backend inventory is received" },
