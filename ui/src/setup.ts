...
export function setupChecklist(input: { connected: boolean; audio: string | null; storage: string | null; deviceCount: number; applicationCount: number; vbCablePairAvailable: boolean }): SetupStep[] {
  return [
    ...
    { id: "storage", label: "Configuration storage", state: input.storage !== null ? "ready" : input.connected ? "needs-attention" : "unavailable", detail: input.storage === "memory" ? "In-memory storage; persistence is not durable" : input.storage === "sqlite" ? "SQLite storage available" : "Not available until a backend snapshot is received" },
    ...
    { id: "vb-cable", label: "VB-Cable loopback", state: input.vbCablePairAvailable ? "ready" : input.connected ? "needs-attention" : "unavailable", detail: input.vbCablePairAvailable ? "Exact active CABLE Output/Input loopback pair detected" : input.connected ? "Select the exact pair only for a deliberate loopback test" : "Unavailable until the backend inventory is received" },
