...
import { writeLayout } from "./layout";
...
function AppContent({ backend = defaultBackend }: { backend?: UiBackend } = {}) {
  ...
  const addLibraryNode = (kind: LibraryNodeKind, position?: { x: number; y: number }) => { const next = appendLibraryNode(draft, kind); const inserted = next.nodes[next.nodes.length - 1]; recordDraftChange(next); if (position) writeLayout(typeof window === "undefined" ? null : window.localStorage, `audiorouter.ui.layout.${draft.id}`, { [inserted.id]: position }); setSelectedNodeId(inserted.id); setActionMessage(`${inserted.name} added to the draft. Review and plan the changes before committing.`); return inserted.id; };
