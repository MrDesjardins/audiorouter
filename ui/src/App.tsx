...
function ProcessorCatalog({ processors, error }: { processors: ProcessorDescriptor[] | null; error: string | null }) {
...
function VirtualDeviceLifecyclePanel({ backend }: { backend: UiBackend }) {
  ...
  const [message, setMessage] = useState<string | null>(null);
  const refresh = () => { void backend.listVirtualDevices().then((items) => { setDevices(items); if (!selectedId && items[0]) { setSelectedId(items[0].id); setBusId(items[0].id); setBusName(items[0].name); } }).catch((error) => setMessage(error instanceof Error ? error.message : "Virtual-device inventory unavailable.")); };
...
export function App({ backend = defaultBackend }: { backend?: UiBackend } = {}) {
  ...
  useEffect(() => {
    let active = true;
    void backend.listPresets().then((items) => { if (active) { setPresets(items); setPresetError(null); } }).catch((error) => { if (active) { setPresets(null); setPresetError(error instanceof Error ? error.message : "Preset catalog unavailable"); } });
    return () => { active = false; };
  }, [backend]);
  useEffect(() => {
    let active = true;
    void backend.listProcessors().then((items) => { if (active) { setProcessors(items); setProcessorError(null); } }).catch((error) => { if (active) { setProcessors(null); setProcessorError(error instanceof Error ? error.message : "Processor catalog unavailable"); } });
    return () => { active = false; };
  }, [backend]);
  useEffect(() => {
    let active = true;
    void backend.listSessions().then((items) => { if (active) { setListedSessions(items); setSessionInventoryError(null); } }).catch((error) => { if (active) { setSessionInventoryError(error instanceof Error ? error.message : "Session inventory unavailable"); setListedSessions(backend.connected ? [] : demoSessions); } });
    return () => { active = false; };
  }, [backend]);
  useEffect(() => {
    let active = true;
      ...
      try {
        ...
        if (result.resyncRequired || result.events.length > 0) {
          ...
          if (active) {
            ...
            refreshApplications();
            refreshDevices();
            void backend.listSessions().then((items) => { if (active) { setListedSessions(items); setSessionInventoryError(null); } }).catch((error) => { if (active) setSessionInventoryError(error instanceof Error ? error.message : "Session inventory unavailable"); });
  ...
  const clearRecoverySafeMode = async () => { setActionMessage("Clearing recovery safe mode..."); try { await backend.clearRecoverySafeMode(uiIdempotencyKey("recovery-clear")); await refresh(); setActionMessage("Recovery safe mode cleared."); } catch (error) { setActionMessage(error instanceof Error ? error.message : "Unable to clear recovery safe mode."); } };
  const createSession = async () => { const name = window.prompt("New session name", "New session")?.trim(); if (!name) return; const id = `session-${Date.now()}`; try { const result = await backend.createSession({ ...demoSession, id, name, revision: 0, nodes: demoSession.nodes.map((node) => ({ ...node, parameters: { ...node.parameters } })), edges: [...demoSession.edges] }, uiIdempotencyKey("session-create")); setCreatedSessions((current) => [...current, result.session]); setSelectedSessionId(result.session.id); setActionMessage(`Created stopped session ${result.session.name}.`); } catch (error) { setActionMessage(error instanceof Error ? error.message : "Unable to create session."); } };
  const duplicateSession = async () => { const id = `session-copy-${Date.now()}`; const name = `${session.name} (copy)`; try { const result = await backend.duplicateSession(session.id, id, name, uiIdempotencyKey("session-duplicate")); setCreatedSessions((current) => [...current, result.session]); setSelectedSessionId(result.session.id); setActionMessage(`Duplicated stopped session ${result.session.name}.`); } catch (error) { setActionMessage(error instanceof Error ? error.message : "Unable to duplicate session."); } };
  ...
  const startSession = async () => { setActionMessage("Starting session..."); try { const result = await backend.startSession(session.id, uiIdempotencyKey("session-start")); await refresh(); setActionMessage(`Session is running (generation ${result.generation}).`); } catch (error) { setActionMessage(error instanceof Error ? error.message : "Unable to start session."); } };
  const stopSession = async () => { setActionMessage("Stopping session..."); try { await backend.stopSession(session.id, uiIdempotencyKey("session-stop")); await refresh(); setActionMessage("Session stopped."); } catch (error) { setActionMessage(error instanceof Error ? error.message : "Unable to stop session."); } };
  ...
  return <div className={`app-shell theme-${theme}`}>{lifecycleActions}<RecorderActions backend={backend} sessionId={session.id} connected={backend.connected} /><RecordingActions recordings={recordings} connected={backend.connected} onRename={renameRecording} onReveal={revealRecording} onRecycle={recycleRecording} /><VirtualDeviceLifecyclePanel backend={backend} />
    <header className="topbar"><div><p className="eyebrow">AudioRouter</p><h1>Routing workspace</h1></div><div className="status-cluster" aria-live="polite"><span className={`status-dot${backend.connected ? "" : " disconnected"}`} aria-hidden="true" /><span>{connectionLabel}</span><span className="status-detail">{statusSummary}</span><label className="theme-picker">Theme<select aria-label="Color theme" value={theme} onChange={(event) => setTheme(event.target.value as ThemeMode)}><option value="dark">Dark</option><option value="light">Light</option><option value="high-contrast">High contrast</option></select></label><button type="button" onClick={refresh}>Reconnect</button></div></header>
      ...
      <main className="main-content"><section className="workspace-title"><div><p className="eyebrow">{sessionRunning ? "Running session" : "Stopped session"}</p><label className="session-name">Session name<input value={draft.name} maxLength={120} disabled={!backend.connected} onChange={(event) => changeSessionName(event.target.value)} /></label><p className="muted">Revision {session.revision} - {backend.connected ? "draft changes require plan and commit" : "changes are presentation-only in this preview"}</p></div><div className="actions"><button type="button" className="secondary" onClick={() => void duplicateSession()} disabled={!backend.connected}>Duplicate</button><button type="button" className="secondary" onClick={() => void deleteSession()} disabled={!backend.connected}>Delete</button><button type="button" className="secondary" onClick={undoDraft} disabled={!backend.connected || draftHistory.past.length === 0}>Undo draft</button><button type="button" className="secondary" onClick={redoDraft} disabled={!backend.connected || draftHistory.future.length === 0}>Redo draft</button><button type="button" className="secondary" onClick={() => { setDraft(session); setDraftHistory({ past: [], future: [] }); setActionMessage("Draft discarded."); }} disabled={!backend.connected}>Discard draft</button><button type="button" className="primary" onClick={() => void planChanges()} disabled={!backend.connected}>Plan changes</button></div></section>
        ...
        <ProcessorCatalog processors={processors} error={processorError} /><PresetCatalog presets={presets} error={presetError} /><PluginScanPanel backend={backend} /><StartupPanel backend={backend} />
        ...
        <section className="panel inspector" aria-labelledby="inspector-heading"><div className="section-heading"><div><p className="eyebrow">Selected node</p><h2 id="inspector-heading">{selectedNode.name}</h2></div><span className="badge">{selectedNode.kind}</span></div><div className="inspector-grid"><label>Node name<input type="text" maxLength={120} value={selectedNode.name} disabled={!backend.connected} onChange={(event) => changeNodeName(event.target.value)} /></label><label>Enabled<input type="checkbox" checked={selectedNode.enabled} disabled={!backend.connected} onChange={(event) => changeNodeFlag("enabled", event.target.checked)} /></label><label>Bypass<input type="checkbox" checked={selectedNode.bypass} disabled={!backend.connected} onChange={(event) => changeNodeFlag("bypass", event.target.checked)} /></label>{selectedNode.kind === "gain" && <label>Gain (dB)<input type="number" min="-60" max="24" step="0.1" value={typeof selectedNode.parameters.gainDb === "number" ? selectedNode.parameters.gainDb : 0} disabled={!backend.connected} onChange={(event) => changeNodeParameter("gainDb", Number(event.target.value))} /></label>}{selectedNode.kind === "mute" && <label>Muted<input type="checkbox" checked={selectedNode.parameters.muted === true} disabled={!backend.connected} onChange={(event) => changeNodeParameter("muted", event.target.checked)} /></label>}{(selectedNode.kind === "gain" || selectedNode.kind === "mute") && <button type="button" className="secondary" onClick={resetNodeParameters} disabled={!backend.connected}>Reset parameters</button>}<button type="button" className="secondary" onClick={duplicateSelectedNode} disabled={!backend.connected}>Duplicate node to draft</button><button type="button" className="secondary" onClick={removeSelectedNode} disabled={!backend.connected}>Remove node from draft</button><button type="button" onClick={() => void togglePrivacyMute()} disabled={!backend.connected} aria-pressed={privacyMuted}>{privacyMuted ? "Privacy mute enabled" : "Enable privacy mute"}</button><p className="muted">{backend.connected ? "Changes are local drafts until Plan changes is committed. Privacy mute is an immediate safety latch." : "Controls are disabled while disconnected. Selection is local presentation state only."}</p></div></section>
