...
function NativeEndpointPanel({ backend, sessionId, devices }: { backend: UiBackend; sessionId: string; devices: DeviceListItem[] }) {
  ...
  }, [devices, captureEndpointId, renderEndpointId]);
  const prepare = async () => {
  ...
  return <section className="panel native-endpoint-panel" aria-labelledby="native-endpoint-heading"><div className="section-heading"><div><p className="eyebrow">Native adapter</p><h2 id="native-endpoint-heading">Endpoint binding</h2></div><span className="badge">explicit</span></div><p className="muted">Select exact active endpoints for this session. Preparation opens stopped clients only; Start session is still required to move audio.</p><label>Capture endpoint<select aria-label="Native capture endpoint" value={captureEndpointId} disabled={!backend.connected || activeCapture.length === 0} onChange={(event) => setCaptureEndpointId(event.target.value)}><option value="">Select capture endpoint</option>{activeCapture.map((device) => <option key={device.id} value={device.id}>{device.name} · {device.format.sampleRateHz} Hz · {device.format.channels} ch</option>)}</select></label><label>Render endpoint<select aria-label="Native render endpoint" value={renderEndpointId} disabled={!backend.connected || activeRender.length === 0} onChange={(event) => setRenderEndpointId(event.target.value)}><option value="">Select render endpoint</option>{activeRender.map((device) => <option key={device.id} value={device.id}>{device.name} · {device.format.sampleRateHz} Hz · {device.format.channels} ch</option>)}</select></label><button type="button" className="secondary" onClick={() => void prepare()} disabled={!backend.connected || !backend.prepareNativeEndpoint || !captureEndpointId || !renderEndpointId}>Prepare native endpoints</button>{message && <p className="muted" role="status" aria-live="polite">{message}</p>}<p className="muted">Requires the authenticated <code>deviceAdministration</code> scope. Endpoint defaults and volume are never changed.</p></section>;
}
