...
export function App({ backend = defaultBackend }: { backend?: UiBackend } = {}) {
  ...
  const toggleConnection = (edgeId: string, enabled: boolean) => { try { setDraft((current) => setDraftConnectionEnabled(current, edgeId, enabled)); setActionMessage(`Connection ${enabled ? "enabled" : "disabled"} in the draft. Review and plan the changes before committing.`); } catch (error) { setActionMessage(error instanceof Error ? error.message : "Unable to change connection state."); } };
