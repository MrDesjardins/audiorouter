...
function ClientsPanel({ backend }: { backend: UiBackend }) {
...
function ProcessorParameterEditor({ node, processors, nodeTypes = null, pluginParameters = null, pluginParameterError = null, connected, onChange }: { node: Node; processors: ProcessorDescriptor[] | null; nodeTypes?: import("@audiorouter/contracts").DiscoveryDocument["nodeTypes"] | null; pluginParameters?: PluginParametersResult | null; pluginParameterError?: string | null; connected: boolean; onChange: (name: string, value: boolean | number | string) => void }) {
  ...
  const parameters = (descriptor?.parameters ?? nodeTypes?.find((nodeType) => nodeType.type === `${wireKind}@1` || nodeType.type === `${node.kind}@1`)?.parameters ?? []).filter((parameter) => !parameter.name.endsWith(":"));
...
function NodeTelemetryPanel({ node, snapshot }: { node: Node; snapshot: import("@audiorouter/contracts").DiagnosticsSnapshot | null }) {
  ...
  const applicationCaptureState = node.kind === "applicationCapture"
    ...
    : null;
  const reason = snapshot?.audio.reason ?? "";
  const normalizedReason = reason.toLocaleLowerCase();
  const endpointOwned = normalizedReason.includes("in use") || normalizedReason.includes("owned");
  const genericState = !snapshot
    ? "stopped"
    : endpointOwned
      ? "endpoint owned by another client"
      : snapshot.audio.state === "unavailable"
        ? "not prepared"
        : observation?.meter && observation.meter.peakDb <= -120
          ? "no signal"
          : observation
            ? "available"
            : "no signal";
  const genericMessage = genericState === "stopped"
    ? "Stopped: start the session to observe this node."
    : genericState === "endpoint owned by another client"
      ? "The exact endpoint is owned by another client; release it before preparing this route."
      : genericState === "not prepared"
        ? "Audio is stopped. Press Play; the message at the top will name any missing device selection."
        : genericState === "no signal"
          ? "No signal: the prepared destination meter is silent or unavailable."
          : null;
  const state = applicationCaptureState?.state ?? (node.kind === "applicationCapture" ? "not prepared" : genericState);
  const message = applicationCaptureState?.detail ?? (node.kind === "applicationCapture" ? "Select the application source and prepare the exact output to monitor its status." : genericMessage);
  return <section className="node-telemetry" aria-labelledby="node-telemetry-heading">
    <div className="section-heading">
      <div><p className="eyebrow">Observed runtime</p><h3 id="node-telemetry-heading">Node telemetry</h3></div>
      <span className="badge">{state}</span>
    </div>
    {!observation || message ? <p className="muted">{message ?? "No prepared meter observation is available for this node."}</p> : <div className="telemetry-values">
...
function FirFilterEditor({ node, backend, disabled, onChange }: { node: Node; backend: UiBackend; disabled: boolean; onChange: (changes: Array<[string, boolean | number | string]>) => void }) {
  ...
  return <div className="node-binding-editor" aria-label="FIR impulse response"><div><p className="eyebrow">Impulse response</p><strong>{typeof node.parameters.fileName === "string" ? node.parameters.fileName : "No impulse response selected"}</strong></div>
    ...
    <small>{message ?? "The response is normalized to unit energy. Use Mix for a blend with the dry signal and Gain for level. Until a file is chosen, audio passes unchanged."}</small>
  </div>;
}
...
function DenoiseLearnEditor({ node, running, liveProfile, disabled, onChange }: { node: Node; running: boolean; liveProfile: string | null; disabled: boolean; onChange: (changes: Array<[string, boolean | number | string]>) => void }) {
  ...
  return <div className="node-binding-editor" aria-label="Denoise noise profile"><div><p className="eyebrow">Noise profile</p><strong>{learning ? "Learning the noise now" : hasProfile ? "A noise profile is stored" : "Teach this tool the noise to remove"}</strong></div>
    {!learning ? <button type="button" className="secondary" disabled={disabled || !running} onClick={() => onChange([["learning", true]])}>{hasProfile ? "Learn again" : "Learn noise"}</button>
      : <button type="button" className="primary" disabled={disabled || !liveProfile} onClick={() => liveProfile && onChange([["noiseProfile", liveProfile], ["learning", false]])}>Stop learning and keep profile</button>}
    <small>{!running ? "Start the route first. Then play only the unwanted noise (for example room tone or fan hiss) for a few seconds while learning." : learning ? (liveProfile ? "Keep only the noise playing for 3–5 seconds, then stop learning. Audio passes unchanged while learning." : "Waiting for the first noise measurement…") : "Reduction sets how aggressively the profile is removed; the remaining floor keeps some noise to avoid unnatural silence."}</small>
  </div>;
}
...
function PhysicalOutputBinding({ devices, value, disabled, onChange, onRefresh }: { devices: DeviceListItem[]; value: string; disabled: boolean; onChange: (value: string) => void; onRefresh: () => void }) {
  const activeRender = sortDevicesAlphabetically(devices.filter((device): device is Extract<DeviceListItem, { state: "active" }> => device.state === "active" && device.direction === "render"));
  return <div className="node-binding-editor" aria-label="Physical output binding"><div><p className="eyebrow">Render destination</p><strong>Choose the headphones, speakers, or virtual output</strong></div><select aria-label="Physical output endpoint" value={value} disabled={disabled || activeRender.length === 0} onChange={(event) => onChange(event.target.value)}><option value="">Select render endpoint</option>{activeRender.map((device) => <option key={device.id} value={device.id}>{device.name} · {device.format.sampleRateHz} Hz · {device.format.channels} ch</option>)}</select><small>{activeRender.length === 0 ? "No active render endpoints are available. Refresh the device list; a reboot is not normally required." : "This selection is used by the session's stopped native render binding."}</small><button type="button" className="secondary" onClick={onRefresh} disabled={disabled}>Refresh available outputs</button></div>;
}
 ...
 */
function PanelMessage({ message, small = false }: { message: string | null | undefined; small?: boolean }) {
  if (!message) return null;
  const error = actionMessageTone(message) === "error";
  const className = error ? "panel-message is-error" : "muted";
  return small
...
function RecorderNodeEditor({ node, disabled, format, onFormatChange }: { node: Node; disabled: boolean; format: import("@audiorouter/contracts").RecorderFileFormat; onFormatChange: (value: import("@audiorouter/contracts").RecorderFileFormat) => void }) {
  const path = typeof node.parameters.path === "string" ? node.parameters.path : "";
...
function AppContent({ backend = defaultBackend }: { backend?: UiBackend } = {}) {
  ...
  const startSession = async (): Promise<boolean> => {
    ...
    try {
      ...
      const applicationSource = applicationOnlyRouteSource(draftRef.current);
      const mixerSources = applicationSource ? null : mixerRouteSources(draftRef.current);
      ...
      if (mixerApplicationRoute && mixerSources) {
        ...
        const unsupported = mixerSources.filter((node) => node.kind !== "physicalInput" && node.kind !== "applicationCapture");
        const microphones = mixerSources.filter((node) => node.kind === "physicalInput");
        if (unsupported.length > 0) {
          setActionMessage(`No audio started. A Mixer that includes an application can combine applications and one input device only. Turn off ${unsupported.map((node) => node.name).join(", ")}, save, then press Play.`);
          return false;
        }
        ...
        const endpoints = readEndpointBindingHint(session.id);
        ...
        if (!(diagnostics.nativeSessionId === session.id && diagnostics.nativeAdapterKind === "multi-input" && ["configured-stopped", "running"].includes(diagnostics.nativeAdapter))) {
          ...
          const bindings = mixerSources.map((node): import("@audiorouter/contracts").NativeMultiInputSourceBinding => node.kind === "physicalInput"
      ...
      const mixedApplicationSource = applicationSource || mixerApplicationRoute ? null : draftRef.current.nodes.find((node) => node.kind === "applicationCapture" && node.enabled) ?? null;
      ...
      if (applicationSource) {
        ...
        const outputEndpointId = readEndpointBindingHint(session.id).renderEndpointId;
        if (!outputEndpointId) {
      ...
      } else if (!mixerApplicationRoute && (diagnostics.nativeSessionId !== session.id || !["configured-stopped", "running"].includes(diagnostics.nativeAdapter))) {
  ...
  return <PluginParameterContext.Provider value={{ parameters: pluginParameters, error: pluginParameterError }}><div className={`app-shell theme-${theme}${compactStatus ? " compact-status" : ""}`}>{lifecycleActions}<RecorderActions backend={backend} sessionId={session.id} connected={backend.connected} recorderStatuses={recorderStatuses} recorderStatusAvailable={recorderStatusAvailable} recorderNodeIds={draft.nodes.filter((node) => node.kind === "recorder").map((node) => node.id)} selectedNodeId={selectedNode.id} onSelectNode={(nodeId) => { setSelectedNodeId(nodeId); setSelectedNodeIds([nodeId]); }} format={recorderFormat} onFormatChange={setRecorderFormat} /><RecordingActions recordings={recordings} connected={backend.connected} busy={recordingMutationBusyState} onRename={renameRecording} onReveal={revealRecording} onRecycle={recycleRecording} /><VirtualDeviceLifecyclePanel backend={backend} onAddVirtualBusNode={addVirtualBusNode} /><VirtualRoutePanel backend={backend} />
    ...
    {(() => {
      const text = actionMessage ? actionMessage : !backend.connected ? `Audio unavailable: ${(snapshot?.status.reason ?? "the backend is disconnected").replace(/\.+$/, "")}. Reconnect to edit or play.` : sessionRunning ? "Audio is running through this session." : "Connect a source to an output, then press Play.";
      const tone = !backend.connected || actionMessageTone(actionMessage) === "error" ? "error" : "info";
      return <div className={`global-action-message is-${tone}`} role={tone === "error" ? "alert" : "status"} aria-live={tone === "error" ? "assertive" : "polite"}>{tone === "error" && <span className="global-action-message-icon" aria-hidden="true">!</span>}<span className="global-action-message-text">{text}</span>{backend.connected && connectionReplacement && actionMessage?.includes("Replace input connection") && <button type="button" className="secondary" onClick={replaceInputConnection}>Replace input connection</button>}{backend.connected && actionMessage?.includes("Devices") && workbenchTab !== "devices" && <button type="button" className="secondary" onClick={() => setWorkbenchTab("devices")}>Open Devices</button>}{backend.connected && actionMessage?.includes("Open Session") && workbenchTab !== "session" && <button type="button" className="secondary" onClick={() => setWorkbenchTab("session")}>Open Session</button>}{backend.connected && actionMessage && <button type="button" className="secondary global-action-message-dismiss" aria-label="Dismiss message" title="Dismiss message" onClick={() => setActionMessage(null)}>×</button>}</div>;
    })()}
