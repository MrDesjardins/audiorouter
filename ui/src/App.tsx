...
import { appendDraftConnection, appendLibraryNode, applyGraphDraft, removeDraftConnection, removeDraftNode, setNodeDraftFlag, setNodeDraftName, setNodeDraftParameter } from "./draft";
...
export function App({ backend = defaultBackend }: { backend?: UiBackend } = {}) {
  ...
  useEffect(() => { setDraft(session); setSelectedNodeId(session.nodes[0]?.id ?? ""); setConnectionSource(""); setConnectionDestination(""); setActionMessage(null); setRouteInspection(null); setPendingWarnings([]); setAcknowledgedWarnings(new Set()); setPendingOperation(null); }, [session]);
  useEffect(() => {
    let active = true;
  ...
  const changeNodeName = (name: string) => { try { setDraft((current) => setNodeDraftName(current, selectedNode.id, name)); setActionMessage("Node name draft updated. Review and plan the changes before committing."); } catch (error) { setActionMessage(error instanceof Error ? error.message : "Unable to rename node."); } };
  ...
  const changeSessionName = (name: string) => { setDraft((current) => ({ ...current, name })); setActionMessage("Session name draft updated. Review and plan the change before committing."); };
  ...
  const addLibraryNode = (kind: "mixer" | "gain" | "mute" | "meter") => { const next = appendLibraryNode(draft, kind); setDraft(next); setSelectedNodeId(next.nodes[next.nodes.length - 1].id); setActionMessage(`${next.nodes[next.nodes.length - 1].name} added to the draft. Review and plan the changes before committing.`); };
  const addConnection = () => { const source = decodePort(connectionSource); const destination = decodePort(connectionDestination); if (!source || !destination) { setActionMessage("Choose an output and input port first."); return; } try { const next = appendDraftConnection(draft, source.nodeId, source.portName, destination.nodeId, destination.portName); setDraft(next); setActionMessage("Connection added to the draft. Review and plan the changes before committing."); } catch (error) { setActionMessage(error instanceof Error ? error.message : "Unable to add connection."); } };
  const removeConnection = (edgeId: string) => { try { setDraft((current) => removeDraftConnection(current, edgeId)); setActionMessage("Connection removed from the draft. Review and plan the changes before committing."); } catch (error) { setActionMessage(error instanceof Error ? error.message : "Unable to remove connection."); } };
  const removeSelectedNode = () => { if (!window.confirm(`Remove node “${selectedNode.name}” and its draft connections?`)) return; try { const next = removeDraftNode(draft, selectedNode.id); setDraft(next); setSelectedNodeId(next.nodes[0]?.id ?? ""); setActionMessage("Node removed from the draft. Review and plan the changes before committing."); } catch (error) { setActionMessage(error instanceof Error ? error.message : "Unable to remove node."); } };
  ...
  return <div className="app-shell">{lifecycleActions}
