...
function NodeCard({ node }: { node: Node }) {
  return (
    <article className="node-card" tabIndex={0} aria-label={`${node.name}, ${node.kind}`}>
    ...
    <div className="app-shell">
      ...
      <div className="workspace-grid">
        ...
        <main className="main-content">
          ...
          <section className="canvas-panel" aria-labelledby="canvas-heading"><div className="section-heading"><div><p className="eyebrow">Signal flow</p><h2 id="canvas-heading">Canvas</h2></div><button type="button" className="secondary">List view</button></div><div className="node-canvas">{demoSession.nodes.map((node) => <NodeCard key={node.id} node={node} />)}</div></section>
