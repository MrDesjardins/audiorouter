...
import { appendDraftConnection, appendLibraryNode, applyGraphDraft, duplicateDraftNode, removeDraftConnection, removeDraftNode, setNodeDraftFlag, setNodeDraftName, setNodeDraftParameter } from "./draft";
...
export function App({ backend = defaultBackend }: { backend?: UiBackend } = {}) {
  ...
  const duplicateSelectedNode = () => { try { const next = duplicateDraftNode(draft, selectedNode.id); const copy = next.nodes[next.nodes.length - 1]; setDraft(next); setSelectedNodeId(copy.id); setActionMessage(`${copy.name} added to the draft without connections. Review and plan the changes before committing.`); } catch (error) { setActionMessage(error instanceof Error ? error.message : "Unable to duplicate node."); } };
