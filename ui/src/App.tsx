...
function ProcessorParameterEditor({ node, processors, nodeTypes = null, pluginParameters = null, pluginParameterError = null, connected, onChange }: { node: Node; processors: ProcessorDescriptor[] | null; nodeTypes?: import("@audiorouter/contracts").DiscoveryDocument["nodeTypes"] | null; pluginParameters?: PluginParametersResult | null; pluginParameterError?: string | null; connected: boolean; onChange: (name: string, value: boolean | number | string) => void }) {
  ...
  const parameters = descriptor?.parameters ?? nodeTypes?.find((nodeType) => nodeType.type === `${wireKind}@1` || nodeType.type === `${node.kind}@1`)?.parameters ?? [];
...
function NativeMultiInputPanel({ backend, sessionId, devices, sessionRunning }: { backend: UiBackend; sessionId: string; devices: DeviceListItem[]; sessionRunning: boolean }) {
  ...
  const prepare = async () => {
    ...
    const parsedGeneration = Number(generation);
    if (!Number.isSafeInteger(parsedGeneration) || parsedGeneration < 1) { setMessage("Generation must be a positive integer."); return; }
    if (endpointIds.length < 2 || endpointIds.length > 8) { setMessage("Select two to eight exact capture endpoints."); return; }
    setBusy(true); setMessage("Preparing exact stopped capture sources...");
    try {
      const result = await backend.prepareNativeMultiInputs(sessionId, parsedGeneration, endpointIds);
      setMessage(`Prepared ${result.captureEndpointIds.length} capture sources for generation ${result.generation}; start the session to activate routing.`);
    } catch (error) { setMessage(formatUiError(error, "Native multi-input preparation failed.")); }
    finally { setBusy(false); }
  };
  return <section className="panel native-multi-input-panel" aria-labelledby="native-multi-input-heading"><div className="section-heading"><div><p className="eyebrow">Many-input routing</p><h2 id="native-multi-input-heading">Multiple capture sources</h2></div><span className="badge">{endpointIds.length}/8</span></div><p className="muted">Select exact active capture endpoints, then verify their explicit order matches the committed physical-input nodes. Preparation is stopped; session start binds the graph’s physical, virtual, recorder, tool, and pre-bound plugin stages.</p><label>Graph generation<input aria-label="Native multi-input graph generation" type="number" min={1} step={1} value={generation} disabled={!backend.connected || sessionRunning || busy} onChange={(event) => setGeneration(event.target.value)} /></label><label>Capture sources<select aria-label="Native multi-input capture endpoints" multiple size={Math.min(8, Math.max(3, activeCapture.length))} value={endpointIds} disabled={!backend.connected || activeCapture.length < 2 || sessionRunning || busy} onChange={(event) => { const selected = new Set(Array.from(event.target.selectedOptions, (option) => option.value)); setEndpointIds((current) => [...current.filter((id) => selected.has(id)), ...Array.from(selected).filter((id) => !current.includes(id))]); }}>{activeCapture.map((device) => <option key={device.id} value={device.id}>{device.format.sampleRateHz} Hz · {device.format.channels} ch · {device.name}</option>)}</select></label>{endpointIds.length > 0 && <fieldset disabled={sessionRunning || busy}><legend>Source order</legend><ol aria-label="Selected multi-input source order">{endpointIds.map((id, index) => <li key={id}><span>{activeCapture.find((device) => device.id === id)?.name ?? id}</span><button type="button" className="secondary" onClick={() => reorder(index, -1)} disabled={index === 0} aria-label={`Move ${id} up`}>Move up</button><button type="button" className="secondary" onClick={() => reorder(index, 1)} disabled={index === endpointIds.length - 1} aria-label={`Move ${id} down`}>Move down</button></li>)}</ol></fieldset>}<button type="button" className="secondary" onClick={() => void prepare()} disabled={!backend.connected || !backend.prepareNativeMultiInputs || endpointIds.length < 2 || sessionRunning || busy}>Prepare stopped multi-input</button>{message && <p className="muted" role="status" aria-live="polite">{message}</p>}<p className="muted">No fallback microphone is selected. If an exact source changes or disappears, the backend stops the worker and requires deliberate rebind.</p></section>;
}
...
function AppContent({ backend = defaultBackend }: { backend?: UiBackend } = {}) {
  ...
  const changeNodeParameter = (name: string, value: boolean | number | string) => { const error = selectedNode.kind === "plugin" ? (() => { const id = Number(name.slice("pluginParameter:".length)); const descriptor = pluginParameters?.parameters.find((parameter) => parameter.parameterId === id); return !descriptor || typeof value !== "number" || !Number.isFinite(value) || value < descriptor.minimum || value > descriptor.maximum ? "Plugin parameter value is outside the worker-provided range" : null; })() : processorParameterError(processors, selectedNode.kind, name, value, snapshot?.discovery?.nodeTypes ?? null); if (error) { setActionMessage(`Draft rejected: ${error}.`); return; } recordDraftChange(setNodeDraftParameter(draft, selectedNode.id, name, value)); setActionMessage("Draft updated. Review and plan the changes before committing."); };
