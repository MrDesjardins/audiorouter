...
function VirtualDevicePanel({ backend }: { backend: UiBackend }) {
  ...
  const [message, setMessage] = useState<string | null>(null);
  const refresh = () => { void backend.listVirtualDevices().then(setDevices).catch((error) => setMessage(error instanceof Error ? error.message : "Virtual-device inventory unavailable.")); };
  useEffect(() => { if (backend.connected) refresh(); else setDevices([]); }, [backend]);
  const createPlan = async () => { setMessage("Planning managed virtual bus..."); try { const result = await backend.planVirtualDevice({ action: "create", id: busId.trim(), name: busName.trim() }); setPlan(result); setMessage(result.availability.reason); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to plan managed virtual bus."); } };
  const applyPlan = async () => { if (!plan) return; setMessage("Applying desired virtual-bus state..."); try { const result = await backend.applyVirtualDevice(plan.planId, `ui-virtual-${Date.now()}`); setPlan(null); setMessage(`${result.availability.reason}; no native endpoint is active in this build.`); refresh(); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to apply virtual-bus plan."); } };
  return <section className="panel virtual-device-panel" aria-labelledby="virtual-device-heading"><div className="section-heading"><div><p className="eyebrow">Managed buses</p><h2 id="virtual-device-heading">Virtual-device inventory</h2></div><button type="button" className="secondary" onClick={refresh} disabled={!backend.connected}>Refresh</button></div>{devices.length === 0 ? <p className="muted">No managed virtual buses are currently listed.</p> : <ul aria-label="Managed virtual devices">{devices.map((device) => <li key={device.id}><strong>{device.name}</strong> <small>{device.enabled ? "enabled" : "disabled"} · {device.availability.reason} · lease {device.leaseOwner ?? "none"}</small></li>)}</ul>}<fieldset disabled={!backend.connected}><legend>Plan a bus</legend><label>Bus ID<input value={busId} maxLength={64} onChange={(event) => setBusId(event.target.value)} /></label><label>Bus name<input value={busName} maxLength={120} onChange={(event) => setBusName(event.target.value)} /></label><button type="button" className="secondary" onClick={() => void createPlan()}>Plan create</button>{plan && <button type="button" className="secondary" onClick={() => void applyPlan()}>Apply planned state</button>}</fieldset>{message && <p className="muted" role="status">{message}</p>}<p className="muted">The backend remains authoritative and requires <code>deviceAdministration</code>. This UI does not create endpoints; the managed driver is unavailable until the native M03 gate is completed.</p></section>;
}
