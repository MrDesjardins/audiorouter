...
export function App({ backend = defaultBackend }: { backend?: UiBackend } = {}) {
  ...
  const changeNodeParameter = (name: string, value: number) => { if (!Number.isFinite(value)) return; setDraft((current) => setNodeDraftParameter(current, selectedNode.id, name, value)); setActionMessage("Draft updated. Review and plan the changes before committing."); };
