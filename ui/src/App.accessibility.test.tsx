...
describe("VB-Cable endpoint selection", () => {
  ...
  it("keeps workspace events bounded to state categories and excludes meters", () => {
    ...
    expect(WORKSPACE_EVENT_CATEGORIES).toContain("devices.bindingInvalidated");
  ...
  it("shows an explicit rebind notice when the native endpoint binding is invalidated", async () => {
    let eventSent = false;
    const subscribe = vi.fn(async (): Promise<EventsSubscribeResult> => {
      if (eventSent) return { backendEpoch: 0, events: [], nextSequence: 1 };
      eventSent = true;
  ...
  it("does not publish a scan result after its directory is edited", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Scan directory" }));
    await screen.findByText("Scanning selected directory...");
    fireEvent.change(directory, { target: { value: "C:\\Plugins\\new" } });
    releaseScan({ directory: "C:\\Plugins\\old", entries: [] });
    await new Promise((resolve) => setTimeout(resolve, 0));
